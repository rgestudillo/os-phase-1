...
export function useFileAdapter(metadata: Pick<Directory.FileContent, 'id'>) {
  ...
  const saveAsNewFile = useMemo(() => async (newName: string, newContent = fileContent?.content ?? "") => {
    // If fileContent is undefined, newContent will be an empty string
    // If fileContent exists but its content is undefined, newContent will still be an empty string
    // If fileContent exists and its content is not undefined, newContent will take its value
    // Assuming createFile and saveFile are imported functions you can call
    const newFileMetadata = await File.createFile({
      name: newName,
      parentId: fileMetadata.parentId, // or another appropriate value
